...
export default function CharDetail() {
  ...
  const [phase, setPhase] = useState(0); // 0=hidden, 1=image centered, 2=settled (image left + panels)
  ...
  // Reset + animation sequence on character change
  useEffect(() => {
    window.scrollTo(0, 0);
    setImgError(false);
    setUiReady(false);
  ...
  // ─── Image animation styles per phase ───
    ...
    <div style={{ background: C.bgDeep, color: C.white, minHeight: "100vh", position: "relative", overflowX: "hidden" }}>
      ...
      <section
        style={{
          ...
          transition: `padding 1.2s ${EASE}, justify-content 1.2s ${EASE}`,
        }}
      >
        {/* Ambient glow — character accent */}
        <div
          style={{
            position: "absolute",
            top: phase === 2 ? "20%" : "35%",
            left: phase === 2 ? (isMobile ? "50%" : "20%") : "50%",
        ...
        {/* ── Phase 1 overlay: Name + tagline (centered, fades out in phase 2) ── */}
        <div
          style={{
            position: "absolute",
            inset: 0,
        ...
        >
          <span
            style={{
              ...
              letterSpacing: "0.3em", textTransform: "uppercase", color: char.color,
              opacity: uiReady ? 1 : 0, transform: uiReady ? "translateY(0)" : "translateY(16px)",
              transition: t(0.2), marginBottom: 12,
            }}
          >
            {char.agency}
          </span>
          <h1
            style={{
              ...
              fontSize: isMobile ? "clamp(48px, 14vw, 64px)" : "clamp(64px, 8vw, 96px)",
              fontWeight: 700, color: C.white, margin: "0 0 12px", lineHeight: 1.1, textAlign: "center",
              opacity: uiReady ? 1 : 0, transform: uiReady ? "translateY(0)" : "translateY(24px)",
              transition: t(0.4),
              textShadow: `0 0 80px ${`color-mix(in oklch, ${char.color} 40%, transparent)`}`,
            }}
          >
            {char.name}
          </h1>
          <p
            style={{
              fontFamily: "var(--f-display-kr)", fontSize: isMobile ? 15 : 18,
              color: C.text70, fontStyle: "italic", margin: 0, lineHeight: 1.6, textAlign: "center",
              opacity: uiReady ? 1 : 0, transform: uiReady ? "translateY(0)" : "translateY(16px)",
              transition: t(0.6),
            }}
          >
        ...
        >
          <Link
            to="/"
            style={{ color: C.text35, textDecoration: "none", fontSize: 12, letterSpacing: "0.08em", transition: "color 0.3s" }}
          >
            &larr; PRIME CITY
          </Link>
        </div>

        {/* ── Main content area: Image + Profile panels ── */}
        <div
          style={{
            width: "100%",
            maxWidth: 1100,
            ...
            justifyContent: phase === 2 ? "flex-start" : "center",
            transition: `gap 1.2s ${EASE}, justify-content 1.2s ${EASE}`,
          }}
        >
          {/* Character image */}
          <div
            style={{
              ...
              position: "relative",
              opacity: uiReady ? 1 : 0,
              transform: uiReady ? "scale(1)" : "scale(0.9)",
              transition: `${imgContainerStyle.transition}, opacity 0.8s ${EASE} 0.1s, transform 0.8s ${EASE} 0.1s, border-color 0.6s ${EASE}`,
            }}
          >
            {hasImage ? (
              <>
                ...
                />
                {/* Vignette overlay on image during phase 1 */}
                <div
                  style={{
                    position: "absolute", inset: 0,
          ...
          {/* Profile panels — slide in from right in phase 2 */}
          <div
            style={{
              flex: 1,
              opacity: phase === 2 ? 1 : 0,
          ...
          >
            {/* Signature tagline — styled quote at top */}
            <p
              style={{
                fontFamily: "var(--f-display-kr)",
            ...
            <p
              style={{
                fontSize: 13, color: C.text45,
                fontFamily: "var(--f-body)", margin: "0 0 20px",
              }}
            >
            ...
            {/* Brief (the main prose description) */}
            {char.brief && (
              <p
                style={{
                ...
                >
                  <span
                    style={{
                      ...
                      letterSpacing: "0.25em", color: char.color,
                  ...
                  <p
                    style={{
                      ...
                      lineHeight: 1.7, color: C.text45, margin: 0, wordBreak: "keep-all",
                    }}
                  >
                    {field.value}
                  </p>
        ...
        >
          <span style={{ fontFamily: "var(--f-display-en)", fontSize: 9, letterSpacing: "0.3em", color: C.text25, textTransform: "uppercase" }}>
            Scroll
          </span>
      ...
      {char.expressions && char.expressions.length > 0 && (
        ...
        >
          <h3 style={{ fontFamily: "var(--f-display-en)", fontSize: 10, letterSpacing: "0.3em", textTransform: "uppercase", color: C.goldText, marginBottom: isMobile ? 20 : 28 }}>
            Expressions
          </h3>
            ...
            {char.expressions.map((key) => {
              ...
              return (
                <div
                  ...
                  style={{
                    aspectRatio: "1/1", background: C.bgCard,
                    border: `1px solid ${C.border06}`, overflow: "hidden",
                    position: "relative", cursor: hasError ? "default" : "pointer",
                ...
                >
                  {!hasError ? (
                    <img
                      src={exprSrc} alt={EXPRESSION_LABELS[key]}
                    ...
                    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, background: `radial-gradient(circle, ${`color-mix(in oklch, ${char.color} 8%, transparent)`}, transparent)` }}>
                      <span style={{ fontSize: isMobile ? 11 : 13, color: C.text25, fontFamily: "var(--f-body)" }}>
                  ...
                  <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: isMobile ? "4px 6px" : "6px 10px", background: `linear-gradient(to top, ${C.bgDeep}, transparent)` }}>
                    <span style={{ fontFamily: "var(--f-body)", fontSize: isMobile ? 9 : 10, color: C.text45, letterSpacing: "0.05em" }}>
                      {EXPRESSION_LABELS[key]}
                    </span>
                  </div>
                </div>
      ...
      >
        {sameAgency.length > 0 && (
          <div style={{ marginBottom: isMobile ? 32 : 48 }}>
            <h3 style={{ fontFamily: "var(--f-display-en)", fontSize: 10, letterSpacing: "0.3em", textTransform: "uppercase", color: C.goldText, marginBottom: isMobile ? 16 : 20 }}>
              Same Agency
            </h3>
            <div style={{ display: "flex", gap: isMobile ? 10 : 16, flexWrap: "wrap" }}>
              {sameAgency.map((c) => (
                <Link
                  key={c.id} to={`/characters/${c.id}`}
                  style={{ textDecoration: "none", padding: isMobile ? "10px 16px" : "12px 20px", background: C.bgCard, border: `1px solid ${C.border06}`, display: "flex", alignItems: "center", gap: 10, transition: "border-color 0.3s, box-shadow 0.3s" }}
      ...
      {lightbox && (
        <div onClick={() => setLightbox(null)} style={{ position: "fixed", inset: 0, zIndex: 9999, background: C.bgOverlay, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }}>
          <div onClick={(e) => e.stopPropagation()} style={{ maxWidth: isMobile ? "90vw" : "60vw", maxHeight: "80vh", position: "relative" }}>
            ...
            <p style={{ textAlign: "center", fontFamily: "var(--f-body)", fontSize: 13, color: C.text55, marginTop: 12 }}>
              {char.name} — {EXPRESSION_LABELS[lightbox.key]}
            </p>
            <button onClick={() => setLightbox(null)} style={{ position: "absolute", top: -12, right: -12, width: 32, height: 32, background: C.bgDeep, border: `1px solid ${C.border10}`, borderRadius: "50%", color: C.text55, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
              ✕
            </button>
          </div>
        </div>
