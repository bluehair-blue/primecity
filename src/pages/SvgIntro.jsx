      ...
      {({ isMobile }) => {
        ...
        return (
          <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            ...
            <div ref={ref} style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3, 1fr)", gap: isMobile ? 16 : 24, opacity: v ? 1 : 0, transform: v ? "translateY(0)" : "translateY(20px)", transition: `all 0.6s ${EASE}` }}>
              {filtered.map((tmpl) => (
                <div
                  ...
                  style={{ background: C.bgCard, border: `1px solid ${C.border06}`, overflow: "hidden", cursor: "pointer", transition: `border-color 0.3s ${EASE}, transform 0.3s ${EASE}`, position: "relative" }}
                ...
                >
                  {/* Safe: SVG generated by our own template functions, not user input */}
            ...
            {selected && (
              <div onClick={() => setSelected(null)} style={{ position: "fixed", inset: 0, zIndex: 9999, background: C.bgOverlay, display: "flex", alignItems: "center", justifyContent: "center", padding: isMobile ? 16 : 32 }}>
                <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 700, maxHeight: "90vh", overflowY: "auto", background: C.bgDeep, border: `1px solid ${C.border06}`, position: "relative" }}>
                  <button onClick={() => setSelected(null)} style={{ position: "sticky", top: 12, float: "right", marginRight: 12, marginTop: 12, width: 32, height: 32, background: C.bgCard, border: `1px solid ${C.border10}`, borderRadius: "50%", color: C.text55, fontSize: 14, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 10 }}>✕</button>
                  {/* Safe: SVG generated by our own template functions */}
                  <div style={{ padding: isMobile ? 16 : 32, paddingTop: isMobile ? 48 : 32, background: "#08081a" }} dangerouslySetInnerHTML={{ __html: selected.generate(selected.sampleParams) }} />
                  ...
                  <div style={{ padding: isMobile ? "16px 16px 24px" : "20px 32px 32px" }}>
                    ...
                    {modalTab === "worker" && (
                      <pre style={{ background: "#0a0a1a", padding: 16, border: `1px solid ${C.border06}`, overflowX: "auto", fontSize: 11, fontFamily: "monospace", color: C.text55, lineHeight: 1.6, margin: 0, whiteSpace: "pre-wrap", wordBreak: "break-all" }}>{selected.workerCode}</pre>
                    )}
                    {modalTab === "prompt" && (
                      <pre style={{ background: "#0a0a1a", padding: 16, border: `1px solid ${C.border06}`, overflowX: "auto", fontSize: 12, fontFamily: "var(--f-body)", color: C.text55, lineHeight: 1.7, margin: 0, whiteSpace: "pre-wrap", wordBreak: "break-all" }}>{selected.promptExample}</pre>
                    )}
                  </div>
                </div>
              </div>
