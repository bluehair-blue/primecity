          ...
          <div style={{ maxWidth: 1100, margin: "0 auto" }}>
              ...
              {tabs.map((tab) => (
                <button
                  ...
                  style={{
                    ...
                    color: category === tab ? C.bgDeep : C.text55,
                    background: category === tab ? C.gold : C.bgCard,
                    border: `1px solid ${category === tab ? C.gold : C.border06}`,
                    cursor: "pointer",
            ...
            >
              {filtered.map((tmpl) => (
                <div
                  key={tmpl.id}
                  onClick={() => { setSelected(tmpl); setModalTab("params"); }}
                  style={{
                    background: C.bgCard,
                ...
                >
                  {/* SVG Preview */}
                  <div
                    style={{ padding: 16, background: C.bgDeep, overflow: "hidden", maxHeight: 260 }}
                    dangerouslySetInnerHTML={{ __html: tmpl.generate(tmpl.sampleParams) }}
                  />
                  {/* Info */}
                  <div style={{ padding: "14px 16px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                      <span style={{ fontFamily: "var(--f-body)", fontSize: 14, fontWeight: 600, color: C.white }}>
                        {tmpl.name}
                      </span>
                      {tmpl.animated && (
                        <span style={{ fontSize: 9, padding: "2px 6px", background: `color-mix(in oklch, ${C.gold} 15%, transparent)`, border: `1px solid ${C.goldText}`, color: C.gold, fontFamily: "var(--f-display-en)", letterSpacing: "0.1em" }}>
                          ANIM
                        </span>
                      )}
                    </div>
                    <span style={{ fontSize: 10, padding: "2px 8px", background: C.bgDeep, border: `1px solid ${C.border05}`, color: C.text35, fontFamily: "var(--f-body)" }}>
                      {tmpl.category}
                    </span>
                  </div>
                </div>
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      ...
                      width: 32, height: 32, background: C.bgCard,
                      border: `1px solid ${C.border10}`, borderRadius: "50%",
                      color: C.text55, fontSize: 14, cursor: "pointer",
                      display: "flex", alignItems: "center", justifyContent: "center",
                      zIndex: 10,
                    }}
                  >
                  ...
                  {/* Full SVG Preview */}
                  <div
                    style={{ padding: isMobile ? 16 : 32, paddingTop: isMobile ? 48 : 32, background: "#08081a" }}
                    dangerouslySetInnerHTML={{ __html: selected.generate(selected.sampleParams) }}
                  />

                  {/* Info */}
                  <div style={{ padding: isMobile ? "16px 16px 0" : "24px 32px 0" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                      <h2 style={{ fontFamily: "var(--f-display-kr)", fontSize: isMobile ? 20 : 24, fontWeight: 700, color: C.white, margin: 0 }}>
                        {selected.name}
                      </h2>
                      <span style={{ fontSize: 10, padding: "2px 8px", background: C.bgCard, border: `1px solid ${C.border05}`, color: C.text35, fontFamily: "var(--f-body)" }}>
                        {selected.category}
                      </span>
                      {selected.animated && (
                        <span style={{ fontSize: 9, padding: "2px 6px", background: `color-mix(in oklch, ${C.gold} 15%, transparent)`, border: `1px solid ${C.goldText}`, color: C.gold, fontFamily: "var(--f-display-en)" }}>
                          ANIMATED
                        </span>
                      )}
                    </div>
                    <p style={{ fontFamily: "var(--f-body)", fontSize: 13, color: C.text45, lineHeight: 1.6, margin: "0 0 20px", wordBreak: "keep-all" }}>
                      {selected.desc}
                    </p>
                  </div>
                      ...
                      {[
                        ...
                        <button
                          ...
                          style={{
                            padding: "10px 20px",
                            fontFamily: "var(--f-body)", fontSize: 12,
                            color: modalTab === tab.key ? C.gold : C.text35,
                            background: "transparent",
                            border: "none",
                            borderBottom: modalTab === tab.key ? `2px solid ${C.gold}` : "2px solid transparent",
                            cursor: "pointer",
                            transition: `all 0.3s ${EASE}`,
                          }}
                        >
                  ...
                  <div style={{ padding: isMobile ? "16px 16px 24px" : "20px 32px 32px" }}>
                    ...
                    {modalTab === "worker" && (
                      <pre style={{
                        background: "#0a0a1a", padding: 16,
                        border: `1px solid ${C.border06}`,
                        overflowX: "auto", fontSize: 11,
                        fontFamily: "monospace", color: C.text55,
                    ...
                    {modalTab === "prompt" && (
                      <pre style={{
                        ...
                        fontFamily: "var(--f-body)", color: C.text55,
