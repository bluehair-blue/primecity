...
**완료**: 사이트 16페이지, 디자인 시스템, 에셋 1,215장+, 보안, 파일 정리, tools/ 파이프라인 개선 (18항목), NSFW 검열 배치 (264/855장), 캐릭터 사인/썸네일 15명 CDN 통일, CharDetail seam cue, **장그루 전용 시네마틱 인트로 (JgrCharDetail v4)**, 사이트 총체적 최적화 ①②③④, CityMap 히트박스 등각 보정, **챗봇 프롬프트 Phase 5 전면 개편 완료 (103개 로어북, 최종 검증 A~E PASS)**, **태블릿 SVG 개편 (10섹션, 13모드, escapeXml 수정, 상대좌표 리팩터링)**, 이미지 에셋 재생성 1,125장 완료 (char_img/)
**미완**: **에덴챗 로어북 삽입 반자동화** (아래 참조), Works 확장, 나머지 캐릭터 사인 이미지, 최적화 ⑤ 접근성(사용자 디자인 판단 대기), **CharDetail 페이지 개편**, svgTemplates.js 고유명사 반영, char_img/ CDN 비교+반영, char_img/ 검열 배치 완료 (conf=0.7, 252장 검열/603장 클린)
...
**접근 후보** (사전 조사 후 확정):
1. **에덴챗 API 직접 호출** — 플랫폼에 로어북 CRUD API가 있으면 최선. 인증 토큰 + REST 호출로 103개 일괄 등록
2. **일괄 임포트 포맷 변환** — 에덴챗이 JSON/CSV 임포트를 지원하면 변환 스크립트 작성
3. **클립보드 순차 파이프라인** — API/임포트 불가 시 fallback. 스크립트가 각 로어북의 (본문, 트리거, fav 범위)를 순서대로 클립보드에 복사 → 유저가 Ctrl+V만 반복
4. **Playwright 반자동** — 유저가 초기 좌표 1회 캘리브레이션 → 이후 자동 반복 (불안정 시 즉시 중단)

**사전 조사 필요 항목**:
- [ ] 에덴챗 로어북 API 존재 여부 (개발자 도구 Network 탭 확인)
- [ ] 에덴챗 로어북 임포트/익스포트 기능 유무
- [ ] 로어북 1개 등록 시 필요한 필드 목록 (이름, 본문, 트리거, fav, 활성화 조건 등)
- [ ] 로어북 등록 순서 제약 (메인 프롬프트 먼저? 의존성?)

**변환 스크립트 준비** (API/임포트 확정 전이라도 미리 가능):
- `tools/edenchat_export.py` — 103개 JSON을 에덴챗 형식으로 변환
